...
const GoalSelection = () => {
  ...
  const cardConfigurations = [
    { title: "Excited to buy, share, or join" },
    { title: "Connected to the story and values behind my brand" },
    { title: "Confident in my expertise or product quality" },
    { title: "Inspired to learn more" },
    { title: "Other" },
  ];
  // const cardConfigurations = [
  //   {
  ...
  useEffect(() => {
    ...
    const mainVibe = (Array.isArray(user?.onboarding?.newVibe) && user?.onboarding?.newVibe.length > 0) ? user?.onboarding?.newVibe[0] : (user?.onboarding?.newVibe || "")
    const tempVibe = cardConfigurations?.find((item) => item.title?.toLowerCase() === mainVibe)?.title?.toLowerCase() || ""
    if (!tempVibe) {
      setCustomType(mainVibe || "");
      setGoal("other");
    } else setGoal(tempVibe);
    setLoginUser(user);
  }, []);
  ...
  const handleNext = () => handleNavigation(3, "/onboard/business-info");
  const handleBack = () => handleNavigation(1, "/onboard/main-goal", true);
          ...
          <div className="2xl:max-w-full mx-auto flex items-center justify-center h-full 2xl:px-[148px] lg:px-[80px] px-[50px]">
            ...
            <div className="flex-1">
              ...
              <h1 className="text-[#0C0829] xl:text-[30px] text-[25px] font-semibold leading-tight !mb-[30px]">
                Amazing,{loginUser?.name ? ` ${loginUser?.name}.` : ""} Since Klque can help you with your goal, when people see your content, what’s the lasting feeling you’d like them to walk away with?
              </h1>
            ...
            {/* <div className="mb-12 mt-4 transition-all duration-300">
                <p className="bg-white mb-4 sm:py-[10px] px-[20px] h-full 2xl:text-[20px] lg:text-[18px] md:text-[20px] sm:text-[20px] font-medium rounded-[12px] border overflow-hidden !leading-[1.45rem] max-w-fit" onClick={() => handleContentTypeSelect("Other")}>Other </p>
